...
O ENIAC não possuia um Conjunto de Instruções da Arquitetura da mesma forma que computadores atuais. As intruções ou programação da máquina era feita diretamente no hardware, reposicionando componentes para mudar o conjunto de instruções que a arquitetura iria realizar.

## Word Size

O tamanho da palavra é a quantidade de dados que um computador consegue processar de uma vez, e em computadores modernos é medida em bits (32 bits, 64 bits). No caso do ENIAC, faz mais sentido medirmos o tamanho da palavra em números decimais do que em bits, pois ele possuia 20 acumuladores e cada um conseguia armazenar um número decimal de até 10 digitos. Ou seja: o ENIAC tinha uma palavra de dados correspondente a um número decimal de 10 dígitos com sinal, e não um tamanho de palavra binário moderno.

## Processor/Family

A arquitetura do ENIAC não corresponde à nenhuma família especefíca. Ele foi uma máquina única que foi projetada especificamente para cálculos númericos. Podemos chamar a família do ENIAC pelo próprio nome dele já que ele foi o único do seu tipo. A família do ENIAC foi composta de apenas um computador de primeira geração (O ENIAC), que utilizava válvulas eletrônicas, com uma arquitetura decimal, baseada em acumuladores e processamento paralelo.

## Memory

O ENIAC tinha como memória principal 20 acumuladores, cada um capaz de armazenar um número decimal de 10 digitos, e eram utilizados tanto para realizar operações aritméticas quanto para armazenar dados e resultados intermediários. Ele também possuía tabelas de função, que armazenavam valores predefinidos (como constantes matemáticas), além de cartões perfurados usados para entrada, saída e armazenamento externo. Em 1953 foi adicionada uma memória de núcleos magnéticos capaz de armazenar 100 palavras.
